...
import { createPurchaseOrder, type LineInput } from '../actions';
...
export function PurchaseOrderForm({ vendors }: { vendors: Vendor[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const id = await createPurchaseOrder({ vendorId, date, tax: Number(tax) || 0, notes, lines: rows.map(({ key, ...rest }) => rest) });
      router.push(`/purchase-management/${id}`);
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="glass-panel overflow-hidden">
        <table className="table-shell">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.key}>
                <td><input value={row.description} onChange={(e) => updateRow(row.key, { description: e.target.value })} className="glass-input" placeholder="Item or service" /></td>
      ...
      <div className="flex items-center justify-between">
        ...
        <button type="submit" disabled={submitting} className="btn-primary ml-auto">Save Purchase Order</button>
      </div>
    </form>
  );
}
