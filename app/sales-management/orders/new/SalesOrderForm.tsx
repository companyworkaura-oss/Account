...
import { createSalesOrder, type LineInput } from '../../actions';
...
export function SalesOrderForm({ customers }: { customers: Customer[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const id = await createSalesOrder({ customerId, date, tax: Number(tax) || 0, notes, lines: rows.map(({ key, ...rest }) => rest) });
      router.push(`/sales-management/orders/${id}`);
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="glass-card grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-white/50">Order Date</label>
      ...
      <div className="flex items-center justify-between">
        ...
        <button type="submit" disabled={submitting} className="btn-primary ml-auto">Save Sales Order</button>
      </div>
    </form>
  );
}
