...
import { createQuotation, type LineInput } from '../../actions';
...
export function QuotationForm({ customers }: { customers: Customer[] }) {
  ...
  const [expiryDate, setExpiryDate] = useState(formatDateInput(new Date(Date.now() + 14 * 24 * 60 * 60 * 1000)));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const id = await createQuotation({ customerId, date, expiryDate, tax: Number(tax) || 0, notes, lines: rows.map(({ key, ...rest }) => rest) });
      router.push(`/sales-management/quotations/${id}`);
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="glass-card grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div>
          <label className="mb-1 block text-xs font-medium text-white/50">Customer</label>
          <select value={customerId} onChange={(e) => setCustomerId(e.target.value)} required className="glass-select">
            <option value="" className="bg-base-800">Select customer…</option>
            {customers.map((c) => (
              <option key={c.id} value={c.id} className="bg-base-800">{c.name}</option>
            ))}
          </select>
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-white/50">Quote Date</label>
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="glass-input" required />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-white/50">Expiry Date</label>
      ...
      <div className="glass-panel overflow-hidden">
        <table className="table-shell">
          <thead>
            <tr><th>Description</th><th className="w-24 text-right">Qty</th><th className="w-32 text-right">Unit Price</th><th className="w-32 text-right">Amount</th><th className="w-10" /></tr>
          </thead>
          <tbody>
            {rows.map((row) => (
              <tr key={row.key}>
                <td><input value={row.description} onChange={(e) => updateRow(row.key, { description: e.target.value })} className="glass-input" placeholder="Product or service" /></td>
                <td><input type="number" min={0} step="0.01" value={row.quantity} onChange={(e) => updateRow(row.key, { quantity: Number(e.target.value) || 0 })} className="glass-input text-right" /></td>
          ...
          <tfoot>
            <tr>
              <td colSpan={2}><button type="button" className="btn-secondary !py-2" onClick={() => setRows((rs) => [...rs, emptyRow()])}><Plus className="h-3.5 w-3.5" /> Add Line</button></td>
      ...
      <div className="flex items-center justify-between">
        ...
        <button type="submit" disabled={submitting} className="btn-primary ml-auto">Save Quotation</button>
      </div>
    </form>
  );
}
