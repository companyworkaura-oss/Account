...
export function InvoiceForm({ customers }: { customers: Customer[] }) {
  ...
  const [dueDate, setDueDate] = useState(formatDateInput(new Date(Date.now() + 15 * 24 * 60 * 60 * 1000)));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/ar/sales-invoices/${id}`);
    } catch (err) {
        ...
        <table className="table-shell">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.key}>
                <td>
                  <input value={row.description} onChange={(e) => updateRow(row.key, { description: e.target.value })} className="glass-input" placeholder="Product or service" />
                </td>
                <td>
