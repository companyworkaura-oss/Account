...
type Invoice = { id: string; invoiceNumber: string; customerId: string; customerName: string; balance: number };
...
export function RecordReceiptButton({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!selected) {
      setError('Select an invoice to receive against.');
      return;
    }
    setSubmitting(true);
    setError(null);
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Record Receipt">
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            >
              ...
              {invoices.map((inv) => (
                <option key={inv.id} value={inv.id} className="bg-base-800">
                  {inv.customerName} · {inv.invoiceNumber} · {formatCurrency(inv.balance)} due
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="mt-2 flex justify-end gap-2">
            ...
            <button type="submit" disabled={submitting} className="btn-primary">Save Receipt</button>
          </div>
        </form>
      </Modal>
    </>
