...
export default async function ARAgingPage() {
  ...
  return (
    <div>
      <PageHeader title="Customer Aging Report" breadcrumb={['Accounts Receivable']} description="Outstanding customer balances grouped by days past due." />
      ...
      <div className="glass-panel overflow-hidden">
        <table className="table-shell">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={5} className="py-10 text-center text-white/40">No outstanding customer balances.</td></tr>
            )}
            {rows.map((r) => (
              <tr key={r.id}>
                <td className="font-medium text-white">{r.customer.name}</td>
